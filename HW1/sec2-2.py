...
X_test = extend_data(1,X_test)
Y_test = test_data['y']



model=np.polyfit(X_train['x'].values,Y_train.values,1)
#first element is theta1 and second element is theta0 so replace them
model[0],model[1]=model[1],model[0]

print(model)
#calculate error for both train and test set
h = X_train.dot(model)
predicted = X_test.dot(model)
def error(h,y):
...
plt.plot(X_train['x'],h)
plt.xlabel('Normalized X')
plt.ylabel('Y')
plt.title(f'Numpy Polyfit')
